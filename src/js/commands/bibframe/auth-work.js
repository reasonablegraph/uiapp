...
cmdCreate('header_public_work_audioCd',{
	action:'addSection',
	opts : jQuery.extend({},commands.header_type.opts,{
		label:key_labels['ea:work:Header_audioCd'],
		class:'sect_work_level_1',
	})
});

cmdCreate('header_public_work_featureFilm',{
	action:'addSection',
	opts : jQuery.extend({},commands.header_type.opts,{
		label:key_labels['ea:work:Header_featureFilm'],
		class:'sect_work_level_1',
	})
});
...
cmdCreate('work_title_preferred_audioCd',{
  action : 'setupField',
  opts : jQuery.extend({},opts_text,{
    'key' : 'dc:title:',
    'label':key_labels['ea:work:Title_Preferred Audio Cd'],
    'lang_select':{
			default_lang:language_default,
			available_languages: languages_avail,
			dist_right: false,
...
/////////////////  Variant Title /////////////////////
cmdCreate('work_variant_title',{
    action : 'setupField',
    opts : jQuery.extend({},opts_text,{
      'key' : 'bf:variantTitle:',
      'label':key_labels['Variant Title'],
      'lang_select':{
				default_lang:language_default,
				available_languages: languages_avail,
				dist_right: false,
...
cmdCreate('work_variant_title_audioCd',{
  action : 'setupField',
  opts : jQuery.extend({},opts_text,{
    'key' : 'bf:variantTitle:',
    'label':key_labels['Variant Title Audio Cd'],
    'lang_select':{
			default_lang:language_default,
			available_languages: languages_avail,
			dist_right: false,
...
/////////////////  Nature Of Content ////////////////
cmdCreate('work_nature_content',{
    action : 'setupField',
    opts : jQuery.extend({},opts_multy_text,{
      'key' : 'bf:natureOfContent:',
      'label':key_labels['NatureOfContent'],
      'show_help' : true,
      repeat_style : 'ordered',
    })
  });
...
////////// Other Agents Associated With Work //////
cmdCreate('other_agents_associated', {
	action : 'setupField',
	opts : jQuery.extend({},opts_multy_text,{
						'key' : 'bf:agentAssociated:',
						'label' : key_labels['OtherAgentsAssociatedWithaWork'],
						repeat_style : 'ordered',
						extend : true,
						extend_command : 'other_agents_associated_extend',
						set_punctuation_as_value:true,
						extend_punctuation : '{{v}}{{#bf:agent:}}{{{v}}}{{/bf:agent:}} ({{#bf:authoritativeLabel:}}{{{v}}}{{/bf:authoritativeLabel:}})',
					})
});

cmdCreate('other_agents_associated_extend', {
  action: 'extend',
  opts:{
    label:key_labels['OtherAgentsAssociatedWithaWork'],
    primary:'agents_associated',
    commands:['agents_associated','agents','relationship_designator'],
  }
});

cmdCreate('agents_associated',{
  action : 'setupField',
  opts : {
    'key' : 'bf:agentAssociated:',
    'type' : 'hidden',
...
cmdCreate.relation(jQuery.extend({},contributor_relation_base ,{
	'cmd_name':'agents',
	'key' : 'bf:agent:',
	'label':key_labels['OtherAgent'],
	"add_button" : false,
}));


cmdCreate.relation({
	'cmd_name':'relationship_designator',
	'object_type':'auth-genre',
	'key' : 'bf:authoritativeLabel:',
	'label':key_labels['RelationshipDesignator'],
  'lang_select':{
		default_lang:language_default_inherited,
		available_languages: languages_avail,
		dist_right: false,
	},
	'search_url':'/prepo/ws/search-subject-form',
	 'extend_commands' : f_auth_relator_extend,
	 'new_button_label' : 'new relationship',
	 'extend_primary_label' : key_labels['authoritative_label'],
});
...
cmdCreate.relation({
	'cmd_name':'manifestation-relation-workOf',
	'object_type':'auth-manifestation',
	'key':{
		'ea:workOf:':'MANIFESTATION'
	},
